...
try {
    ...
    $plan_type = $input['plan']; // 'radio' o 'tv'
    $billing_type = $input['billingType']; // 'monthly' o 'annual'
    $plan_key = $plan_type . '_' . $billing_type;

    $stmt = $pdo->prepare("SELECT * FROM plans WHERE plan_key = ? AND is_active = TRUE");
    $stmt->execute([$plan_key]);
    $plan_data = $stmt->fetch(PDO::FETCH_ASSOC);

    if (!$plan_data) {
        throw new Exception('Plan no válido o no está activo.');
    }
    ...
    error_log("💰 Precio obtenido de la BD: $amount para plan_key: $plan_key");
    ...
    $stmt = $pdo->prepare("
        INSERT INTO orders (
            ...
            project_name, plan_type, billing_type, plan_name, plan_id, amount, status, created_at
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', NOW())
    ");
