const v="/php/api";let a=[],d={},r=null;async function h(){try{const t=await(await fetch(`${v}/get-tutorials.php`)).json();if(t.success){a=t.categories||[];const o=t.tutorialsByCategory||{};d={};for(let i in o)d[parseInt(i)]=o[i];console.log("Loaded categories:",a),console.log("Loaded tutorialsByCategory:",d),a.length>0?(p(),f(),x()):g("No hay tutoriales disponibles")}else g(t.error||"Error al cargar tutoriales")}catch(e){console.error("Error loading tutorials:",e),g("Error de conexión al cargar tutoriales")}}function p(){const e=document.getElementById("categories-tabs");if(!e){console.error("categories-tabs container not found");return}e.innerHTML=a.map((t,o)=>`
            <button
                id="tab-${t.slug}"
