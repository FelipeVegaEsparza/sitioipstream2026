const y="/php/api";let d=[],g={},r=null;async function h(){try{const t=await(await fetch(`${y}/get-tutorials.php`)).json();t.success?(d=t.categories||[],g=t.tutorialsByCategory||{},d.length>0?(p(),f(),x()):u("No hay tutoriales disponibles")):u(t.error||"Error al cargar tutoriales")}catch(e){console.error("Error loading tutorials:",e),u("Error de conexión al cargar tutoriales")}}function p(){const e=document.getElementById("categories-tabs");if(!e){console.error("categories-tabs container not found");return}e.innerHTML=d.map((t,n)=>`
            <button
                id="tab-${t.slug}"
        ...
        `).join(""),document.querySelectorAll(".category-tab").forEach(function(t){t.addEventListener("click",function(){const n=parseInt(t.getAttribute("data-category-id")||"0");console.log("Category clicked:",n),b(n)})}),d.length>0&&(r=d[0].id,console.log("Initial category set:",r))}function b(e){console.log("Switching to category:",e),r=e,document.querySelectorAll(".category-tab").forEach(function(t){parseInt(t.getAttribute("data-category-id")||"0")===e?t.classList.add("active"):t.classList.remove("active")}),f()}function f(){const e=document.getElementById("tutorials-content");if(!e||r===null)return;const t=d.find(function(o){return o.id===r});if(!t)return;const n=g[r]||[],i={beginner:"Básico",intermediate:"Intermedio",advanced:"Avanzado"},s={beginner:"blue",intermediate:"yellow",advanced:"red"};e.innerHTML=`
            ...
            `:`
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    ${n.map(function(o){const a=v(o.video_url),c=i[o.difficulty]||"Básico",m=s[o.difficulty]||"blue";return`
