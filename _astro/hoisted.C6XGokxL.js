const y="/php/api";let d=[],g={},r=null;async function h(){try{const t=await(await fetch(`${y}/get-tutorials.php`)).json();t.success?(d=t.categories||[],g=t.tutorialsByCategory||{},d.length>0?(p(),f(),x()):u("No hay tutoriales disponibles")):u(t.error||"Error al cargar tutoriales")}catch(e){console.error("Error loading tutorials:",e),u("Error de conexión al cargar tutoriales")}}function p(){const e=document.getElementById("categories-tabs");e&&(e.innerHTML=d.map((t,n)=>`
            <button
                id="tab-${t.slug}"
